    ...
    <div class="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            @if (auth()->user()?->can('dashboard.settings') || auth()->user()?->can('settings.manage') || auth()->user()?->hasRole('Super Admin'))
                <a href="{{ route('dashboard.settings.index') }}" wire:navigate class="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-bold transition-all border border-slate-200 dark:border-slate-700">
