                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        @foreach ($lines as $index => $line)
                            <tr>
                                ...
                                <td class="px-4 py-3">
                                    <select wire:model="lines.{{ $index }}.account_id" wire:change="checkBudgetWarning({{ $index }})" class="w-full px-2.5 py-1.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-mono dark:text-slate-100 focus:ring-2 focus:ring-indigo-500">
                                    ...
                                    @if(isset($budgetWarnings[$index]))
                                        <div class="mt-1 px-2 py-1 rounded text-[11px] font-semibold flex items-center gap-1.5 {{ $budgetWarnings[$index]['status'] === 'exceeded' ? 'bg-rose-100 dark:bg-rose-950/70 text-rose-700 dark:text-rose-300 border border-rose-300' : 'bg-amber-100 dark:bg-amber-950/70 text-amber-800 dark:text-amber-300 border border-amber-300' }}">
                                            <svg class="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                                            </svg>
                                            <span>{{ $budgetWarnings[$index]['message'] }}</span>
                                        </div>
                                    @endif
                                </td>
                                <td class="px-4 py-3">
                                    <select wire:model="lines.{{ $index }}.unit_id" wire:change="checkBudgetWarning({{ $index }})" class="w-full px-2.5 py-1.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-semibold dark:text-slate-100 focus:ring-2 focus:ring-indigo-500">
                                ...
                                <td class="px-4 py-3">
                                    <div x-data="{
                                        ...
                                        <input 
                                            ...
                                            x-model="formatted" 
                                            @input="onInput($event)"
                                            class="w-full px-2.5 py-1.5 text-right font-mono font-bold bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-indigo-600 dark:text-indigo-400 focus:ring-2 focus:ring-indigo-500"
                                            placeholder="0"
                                        />
                                        <template x-if="scale">
                                            <div class="text-[9px] font-semibold text-indigo-500 dark:text-indigo-400 text-right mt-0.5 tracking-tighter truncate" x-text="scale"></div>
                                        </template>
                                    </div>
                                </td>
                                <td class="px-4 py-3">
