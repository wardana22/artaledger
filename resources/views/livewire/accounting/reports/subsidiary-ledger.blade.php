    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2">
            @if($selectedAccountId)
                <a 
                    href="{{ route('accounting.reports.export.pdf', ['type' => 'subsidiary-ledger', 'account_id' => $selectedAccountId, 'start_date' => $startDate, 'end_date' => $endDate, 'unit' => $unitFilter]) }}" 
                    target="_blank"
                    class="px-3.5 py-1.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold shadow-md shadow-indigo-500/20 flex items-center gap-2 transition-all">
