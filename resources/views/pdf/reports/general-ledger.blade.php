@extends('pdf.reports.layout', ['title' => 'Buku Besar'])

@section('report_title', 'BUKU BESAR (GENERAL LEDGER)')
...
@section('content')
    <div style="background-color: #f1f5f9; border: 1px solid #cbd5e1; border-radius: 4px; padding: 6px 10px; margin-bottom: 10px; font-size: 9.5px;">
        <table style="width: 100%;">
            <tr>
                <td style="width: 60%;">
                    <strong>Akun Header:</strong> <span class="font-mono font-bold">{{ $account->code }}</span> - {{ $account->name }}<br>
    ...
    <table class="data-table">
        <thead>
            <tr>
                ...
                <th style="width: 11%;" class="text-left">NO. JURNAL</th>
                <th style="width: 11%;" class="text-left">NO. BUKTI</th>
                <th style="width: 6%;" class="text-center">UNIT</th>
                <th style="width: 28%;" class="text-left">KETERANGAN TRANSAKSI</th>
        ...
        <tbody>
            ...
            @forelse($reportLines as $line)
                <tr>
                    ...
                    <td class="text-left">
                        {{ $line['description'] }}
                        <div style="font-size: 7.5px; color: #64748b; font-family: monospace;">[{{ $line['account_code'] }} - {{ $line['account_name'] }}]</div>
                    </td>
