...
6. **Neraca (Balance Sheet)**: `/accounting/reports/balance-sheet` ([BalanceSheet.php](file:///d:/Belajar%20Laravel/artaledger/app/Livewire/Accounting/Reports/BalanceSheet.php))
   - Menggunakan mekanisme *Rollover-Aware Querying*: Mendeteksi keberadaan Jurnal Saldo Awal (`SA`) tahun berjalan. Jika ada rollover, akumulasi mutasi akun neraca dan laba periode berjalan dihitung dari awal tahun berjalan (`{$asOfYear}-01-01` s.d. cut-off date), mencegah *double-counting* aset/kewajiban dari tahun-tahun sebelumnya yang telah ditutup buku.
   - Sinkronisasi presisi dengan [FinancialReportPdfService.php](file:///d:/Belajar%20Laravel/artaledger/app/Domain/Accounting/Services/FinancialReportPdfService.php) dan [FinancialReportExcelService.php](file:///d:/Belajar%20Laravel/artaledger/app/Domain/Accounting/Services/FinancialReportExcelService.php), lengkap dengan penyerapan selisih pembulatan sen historis sehingga posisi neraca senantiasa seimbang (Total Aset = Total Liabilitas & Ekuitas, selisih Rp 0,00).
   - Pengujian terverifikasi via [BalanceSheetRolloverTest.php](file:///d:/Belajar%20Laravel/artaledger/tests/Feature/Accounting/BalanceSheetRolloverTest.php).
...
9. **Saldo Awal**: `/accounting/reports/opening-balance` ([OpeningBalanceIndex.php](file:///d:/Belajar%20Laravel/artaledger/app/Livewire/Accounting/OpeningBalance/OpeningBalanceIndex.php))
   ...
   - Sinkronisasi ekspor PDF dan Excel multi-mode via [FinancialReportPdfService.php](file:///d:/Belajar%20Laravel/artaledger/app/Domain/Accounting/Services/FinancialReportPdfService.php) dan [FinancialReportExcelService.php](file:///d:/Belajar%20Laravel/artaledger/app/Domain/Accounting/Services/FinancialReportExcelService.php).
10. **Laporan Umur Piutang & Hutang (Aging AR/AP)**: `/accounting/reports/aging` ([AgingReport.php](file:///d:/Belajar%20Laravel/artaledger/app/Livewire/Accounting/Reports/AgingReport.php))
    ...
    - Fitur canggih:
      ...
      - **Dual-Tab Controller & Smart Filters**: Beralih instan antara Piutang Usaha (AR) dan Hutang Usaha (AP), filter akun saldo nol, dan toggle **Tampilkan Invoice Lunas** (`includePaidInvoices`) yang menampilkan faktur yang sudah lunas dengan badge status `Lunas` serta tombol akses langsung `Riwayat Bayar` untuk audit atau koreksi pembatalan pelunasan.
...
- **Ekspor Dokumen PDF**: [FinancialReportPdfController.php](file:///d:/Belajar%20Laravel/artaledger/app/Http/Controllers/FinancialReportPdfController.php) $\rightarrow$ [FinancialReportPdfService.php](file:///d:/Belajar%20Laravel/artaledger/app/Domain/Accounting/Services/FinancialReportPdfService.php) (Layout A4 landscape cetak siap tanda tangan untuk laporan berkala & **Rekapitulasi Daftar Jurnal** `[pdf.reports.journal-register]`, serta Layout A4 portrait untuk **Bukti Memorial / Voucher Jurnal** `[pdf.reports.journal-voucher]`)
- **Ekspor Dokumen Excel**: [FinancialReportExcelController.php](file:///d:/Belajar%20Laravel/artaledger/app/Http/Controllers/FinancialReportExcelController.php) $\rightarrow$ [FinancialReportExcelService.php](file:///d:/Belajar%20Laravel/artaledger/app/Domain/Accounting/Services/FinancialReportExcelService.php) (Spreadsheet multi-kolom bucket dengan format akuntansi terpadu, termasuk **Rekapitulasi Daftar Jurnal Transaksi**)
...
- **Form Anggaran & Alokasi Bulanan**: `/accounting/budgets/create` & `/{id}/edit` $\rightarrow$ [BudgetForm.php](file:///d:/Belajar%20Laravel/artaledger/app/Livewire/Accounting/Budgets/BudgetForm.php) $\rightarrow$ [budget-form.blade.php](file:///d:/Belajar%20Laravel/artaledger/resources/views/livewire/accounting/budgets/budget-form.blade.php)
  ...
  - Dilengkapi *Live Currency Separator Masking* (format titik ribuan, jutaan, miliaran) dan indikator skala verbal otomatis (misal: "Rp 8 Miliar", "Rp 666,67 Juta") pada input plafon tahunan dan alokasi 12 bulan tanpa memotong angka nominal.
...
- [tests/Feature/Accounting/ImportJournalTest.php](file:///d:/Belajar%20Laravel/artaledger/tests/Feature/Accounting/ImportJournalTest.php) (Validasi Impor Jurnal Excel: Pengabaian Otomatis Baris Tanpa Kode Akun, Deteksi Unit Kerja, dan Validasi Akun Posting)
...
- [tests/Feature/Accounting/JournalRegisterExportTest.php](file:///d:/Belajar%20Laravel/artaledger/tests/Feature/Accounting/JournalRegisterExportTest.php) (Validasi Ekspor Rekapitulasi Daftar Jurnal Transaksi format PDF Landscape & Excel XLSX)
